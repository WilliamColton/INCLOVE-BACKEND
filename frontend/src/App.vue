...
async function register() {
  ...
  try {
    ...
    log(body?.message ? `注册成功：${body.message}` : '注册成功。')
  } catch (err) {
...
async function login() {
  ...
  try {
    ...
    const data = body?.data
    afterLogin(payload.userId, data?.jwtToken)
    log(body?.message ? `登录成功：${body.message}` : '登录成功。')
  } catch (err) {
...
async function openConversation() {
  ...
  try {
    ...
    ensureApiResponse(postRes, '开启会话')
    const res = await axios.get(`${restBase.value}/conversation/userIdAndPeerUserId`, {
      params: { peerUserId: peerId },
      headers,
    })
    const body = ensureApiResponse(res, '查询会话')
    const payload = body?.data ?? body
    const convoId = payload?.id || payload?.conversationId || ''
    if (!convoId) {
      log('未找到会话，请确认双方已经注册。')
      return
    }
